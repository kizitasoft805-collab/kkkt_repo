...
def get_tithes_monthly_analysis():
    """
    Analyzes Tithes by month (date_given) for the current year (using the timezone 'now').
    Creates a combined line+bar chart for monthly Tithe sums, identifies highest/lowest months,
    and summarizes:
      - total Tithes across all years,
      - total Tithes in the current year,
      - total Tithes in the current month,
      - highest/lowest Tithe month in the current year,
      - single descriptive text combining all.

    Returns JSON for Chart.js + textual summary.
    """

    # 1) Current year from system timezone
    current_year = now().year

    # 2) Query Tithes in the current year, group by month
    #    We'll sum amounts by the month of date_given.
    monthly_tithes_qs = (
        Tithe.objects.filter(date_given__year=current_year)
        .annotate(month=ExtractMonth("date_given"))    # e.g. month=1..12
        .values("month")
        .annotate(total=Sum("amount"))
        .order_by("month")
    )
    ...
    # We'll track highest and lowest months
    highest_month = None
    highest_sum = 0
    lowest_month = None
    lowest_sum = None

    # Build label/data arrays from 1..12 to keep zero months displayed if you want:
    # If you prefer to only show months that have data, just loop over monthly_tithes_qs.
    # We'll do "only months that appear" approach here:
    for entry in monthly_tithes_qs:
        m = entry["month"] or 0
        total_amt = entry["total"] or 0
        months_labels.append(str(m))  # e.g. "1", "2", "3"
        sums_data.append(float(total_amt))

        # highest/lowest
        if total_amt > highest_sum:
            highest_sum = total_amt
            highest_month = m

        if lowest_sum is None or total_amt < lowest_sum:
            lowest_sum = total_amt
            lowest_month = m

    # Summaries
    # A) total Tithes across *all* years
    overall_tithes = Tithe.objects.aggregate(s=Sum("amount"))["s"] or 0

    # B) total Tithes in the current year
    current_year_tithes = (
        Tithe.objects.filter(date_given__year=current_year)
    ...
    # C) total Tithes in the current month
    current_month = now().month
    current_month_tithes = (
        Tithe.objects.filter(date_given__year=current_year, date_given__month=current_month)
        .aggregate(s=Sum("amount"))["s"]
        or 0
    )
    ...
    if not months_labels:
        # No data in the current year
        combined_description = (
            f"There are no Tithes recorded for the current year ({current_year}). "
            f"So far, no monthly data is available."
        )
    else:
        # We have at least some monthly data
        combined_description = (
            f"In the current year ({current_year}), we have Tithes recorded in certain months. "
            f"The month with the highest Tithes is Month {highest_month} with {highest_sum} TZS, "
            f"while the lowest Tithes month is Month {lowest_month} with {lowest_sum} TZS. "
            f"Across all years, the total Tithes amount to {overall_tithes} TZS. "
            f"So far this year, {current_year_tithes} TZS have been contributed. "
            f"This current month has gathered {current_month_tithes} TZS. "
            f"Monitoring these monthly Tithe trends helps in financial planning and forecasting."
        )

    # Return data for combined line+bar chart, plus the single descriptive text
    return json.dumps({
        "months_labels": months_labels,    # e.g. ["1","2","3","4"]
        "sums_data": sums_data,           # e.g. [120000, 50000, 80000, 0]
